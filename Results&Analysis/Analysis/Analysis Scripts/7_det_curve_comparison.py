...
def plot_combined_det_curves():
    ...
    for idx, (csv_file, epoch, aug) in enumerate(file_infos):
        ...
        if oob:
            label = f"{label} ⚠ rajojen ulkopuolella"
    ...
    ax.set_title('DET-käyrät (wav2vec2, 1 epookki) – augmentaatiovertailu', fontsize=14)

    legend_title = 'Augmentaatio'
    if any_oob:
        legend_title = legend_title + ' (⚠ = käyrä osittain rajojen ulkopuolella)'
