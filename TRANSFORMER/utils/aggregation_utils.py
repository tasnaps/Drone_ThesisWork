...
def aggregate_chunk_predictions(chunk_predictions, chunk_probabilities, chunk_metadata, aggregation_method="max_confidence"):
    """
    Aggregate predictions from multiple chunks of the same file.

    Args:
        chunk_predictions: List of binary predictions (0 or 1)
        chunk_probabilities: List of drone probabilities (0.0 to 1.0)
        chunk_metadata: List of (start_time, end_time, chunk_idx) tuples
        aggregation_method: How to combine chunk results

    Returns:
        Tuple of (final_prediction, final_probability, aggregation_info)
    """
    if len(chunk_predictions) == 1:
        return chunk_predictions[0], chunk_probabilities[0], {"method": "single_chunk"}

    chunk_probs = np.array(chunk_probabilities)
    chunk_preds = np.array(chunk_predictions)

    if aggregation_method == "max_confidence":
        # Use the prediction from the chunk with highest confidence (furthest from 0.5)
        confidences = np.abs(chunk_probs - 0.5)
        max_conf_idx = np.argmax(confidences)
        final_prediction = chunk_preds[max_conf_idx]
        final_probability = chunk_probs[max_conf_idx]

        info = {
            "method": "max_confidence",
            "chosen_chunk": max_conf_idx,
            "chosen_confidence": confidences[max_conf_idx],
            "chunk_probabilities": chunk_probs.tolist(),
            "num_drone_chunks": np.sum(chunk_preds),
            "num_total_chunks": len(chunk_preds)
        }

    elif aggregation_method == "majority_vote":
        # Simple majority vote
        final_prediction = 1 if np.mean(chunk_preds) > 0.5 else 0
        final_probability = np.mean(chunk_probs)

        info = {
            "method": "majority_vote",
            "drone_chunk_ratio": np.mean(chunk_preds),
            "avg_probability": final_probability,
            "num_drone_chunks": np.sum(chunk_preds),
            "num_total_chunks": len(chunk_preds)
        }

    elif aggregation_method == "any_drone":
        # If ANY chunk detects drone, classify as drone
        final_prediction = 1 if np.any(chunk_preds) else 0
        # Use max probability for final probability
        final_probability = np.max(chunk_probs)

        info = {
            "method": "any_drone",
            "max_probability": final_probability,
            "num_drone_chunks": np.sum(chunk_preds),
            "num_total_chunks": len(chunk_preds)
        }

    elif aggregation_method == "weighted_average":
        # Weight by confidence - more confident predictions have more influence
        confidences = np.abs(chunk_probs - 0.5)
        weights = confidences / np.sum(confidences) if np.sum(confidences) > 0 else np.ones_like(confidences) / len(confidences)

        final_probability = np.average(chunk_probs, weights=weights)
        final_prediction = 1 if final_probability > 0.5 else 0

        info = {
            "method": "weighted_average",
            "weighted_probability": final_probability,
    ...
    return final_prediction, final_probability, info
